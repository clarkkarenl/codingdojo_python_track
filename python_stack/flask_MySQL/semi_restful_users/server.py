...
# DONE - GET request to /users - calls the index method to display all the users.
...
# DONE - GET request to /users/new - calls the new method to display a form allowing users to create a new user.
...
# DONE - GET /users/<id> - calls the show method to display the info for a particular user with given id.
...
# DONE - GET /users/<id>/destroy - calls the destroy method to remove a particular user with the given id. Have this redirect back to /users once deleted.
@app.route('/users/<id>/destroy', methods=['POST'])
def destroy(user_id):
    delete_user_query = "DELETE FROM `users` WHERE `users`.`id` = :field_one;"
    delete_user_data = {'field_one' : user_id}
